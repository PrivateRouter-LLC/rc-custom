...
if ! opkg list-installed | grep -q "^lxc " && [ -d /opt/docker2 ]; then
    ...
    opkg install lxc lxc-attach lxc-auto lxc-autostart lxc-cgroup lxc-checkconfig lxc-common lxc-config lxc-configs lxc-console lxc-copy lxc-create lxc-destroy lxc-device lxc-execute lxc-freeze lxc-hooks lxc-info lxc-init lxc-ls lxc-monitor lxc-monitord lxc-snapshot lxc-start lxc-stop lxc-templates lxc-top lxc-unfreeze lxc-unprivileged lxc-unshare lxc-user-nic lxc-usernsexec lxc-wait liblxc luci-app-lxc luci-i18n-lxc-en rpcd-mod-lxc

    log_say "Create LXC directories and files"
    [ ! -d /opt/docker2/compose/lxc ] && mkdir -p /opt/docker2/compose/lxc

    if [ -f "${SCRIPT_DIR}/lxc/default.conf" ]; then
        log_say "Copying default.conf to /etc/lxc"
        [ -f /etc/lxc/default.conf ] && mv /etc/lxc/default.conf /etc/lxc/default.conf.bak
        cat "${SCRIPT_DIR}/lxc/default.conf" /etc/lxc/default.conf
    fi

    if [ -f "${SCRIPT_DIR}/lxc/lxc.conf" ]; then
        log_say "Copying lxc.conf to /etc/lxc"
        [ -f /etc/lxc/lxc.conf ] && mv /etc/lxc/lxc.conf /etc/lxc/lxc.conf.bak
        cp "${SCRIPT_DIR}/lxc/lxc.conf" /etc/lxc/lxc.conf
    fi

    if [ -f "${SCRIPT_DIR}/lxc/lxc-auto" ]; then
        log_say "Copying lxc-auto to /etc/init.d"
        [ -f /etc/init.d/lxc-auto ] && mv /etc/init.d/lxc-auto /etc/init.d/lxc-auto.bak
        cp "${SCRIPT_DIR}/lxc/lxc-auto" /etc/init.d/lxc-auto
